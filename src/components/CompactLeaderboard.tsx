...
export const CompactLeaderboard: React.FC<CompactLeaderboardProps> = ({ 
  ...
  const getDisplayScores = () => {
    ...
    // Always show top score if not the current user
    if (currentIndex > 0) {
      displayScores.push(currentScores[0]);
    }
    
    // Show score above current user if exists
    if (currentIndex > 1) {
    ...
    // Show score below current user if exists
  ...
  return (
    <div className={cn(
      "fixed top-4 left-4 rounded-lg shadow-lg w-80",
      ...
      <div className="p-4">
        ...
        <div className="space-y-1">
          {displayScores.map((score, index, array) => {
            ...
            // Add ellipsis if there's a gap in positions
